package com.tamashenning.themasks.tracker;
...
public class GooglyTracker
{
    ...
    public boolean shouldRender()
    {
        ...
        for(String s : TheMasks.config.nameOverride)
        {
            if(s.equals(name))
            {
                return true;
        ...
        if(TheMasks.config.entityOverrideChanceParsed.containsKey(parent.getType().getRegistryName()))
        {
            return renderChance < TheMasks.config.entityOverrideChanceParsed.get(parent.getType().getRegistryName()) / 100F;
        }

        return renderChance < (TheMasks.config.masksChance / 100F);
    }
}
